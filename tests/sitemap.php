<?php
/**
 * The sitemap: what gets published, and what is refused.
 *
 * The fetch itself is not covered — it is `wp_remote_get` and a transient — but every rule the
 * plugin applies to the answer is, which is where the decisions are.
 *
 * @package SahajAtlas
 */
...
$sahaj_body = array(
	...
	'urls'      => array(
		...
		// ⚠ A URL on somebody else's domain. The endpoint answers what the CLIENT owns, and one key
		// shared between two sites — or a mis-set `canonical.embed` — puts a foreign host in the
		// list. Publishing it is a cross-site claim, which is what search engines treat it as.
...
// A `&` in a query string is the realistic case: a query-routing canonical is
// `…/find-a-class/?atlas=/gb/london`, and a second parameter makes it `…&…`. Raw, that is invalid
// XML and the whole document fails to parse — taking every other URL with it.
...
// ⚠ `blog_public = 0` is the site owner saying "do not index me". Adding a sitemap line there would
// override a decision that is theirs, in the one file that exists to express it.
sahaj_is( 'and says nothing when the site asks not to be indexed', "User-agent: *\nDisallow:\n", sahaj_atlas_robots_txt( "User-agent: *\nDisallow:\n", false ) );

/*
 * ⚠ With nothing to publish, an SEO plugin's index must gain NOTHING. An index entry pointing at a
 * sitemap that 404s (which is what `sahaj_atlas_maybe_serve_sitemap()` does for an empty set) is a
 * broken link handed straight to a crawler, and it is the state a site is in for the five minutes
 * after any failed fetch.
 */
...
// The Yoast and Rank Math adapters are the same one-string append, deliberately — see the module
// docblock for why there is no renderer for either.
